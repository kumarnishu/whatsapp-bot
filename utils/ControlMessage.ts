...
import { FlowNode, IMenuTracker } from "../types/flow.types";
...
export const ControlMessage = async (client: Client, msg: WAWebJS.Message, customer_name?: string) => {
    ...
    let sendingMessage = ""
    let trackers = await MenuTracker.find({ phone_number: from?._serialized, bot_number: msg.to }).populate('flow')
